...
async def list_docs_from_db(
    ...
    docs = session.query(
        ...
        FileDocModel.kb_name.ilike(kb_name)
    )
    if file_name:
...
@with_async_session
async def count_files_from_db(
        session,
        kb_name: str
) -> str:
    return session.query(
        KnowledgeFileModel
    ).filter(KnowledgeFileModel.kb_name.ilike(kb_name)).count()
...
async def list_files_from_db(
    ...
    files = session.query(
        KnowledgeFileModel
    ).filter(KnowledgeFileModel.kb_name.ilike(kb_name)).all()
...
async def add_file_to_db(
    ...
    stmt = select(KnowledgeBaseModel).where(
        KnowledgeBaseModel.kb_name == kb_file.kb_name
    )
...
@with_async_session
async def delete_files_from_db(
        session,
        knowledge_base_name: str
):
    session.query(
        KnowledgeFileModel
    ).filter(KnowledgeFileModel.kb_name.ilike(knowledge_base_name)).delete(
